...
Then('The Country filter is available', async function () {
    await this.page.locator(filtersSelectors.country_filters).isVisible()
})

When('The user selects {string} from the {string} filter list on the left panel', async function (filterName, filterType) {
    await this.page.check(`${filtersSelectors.filter_type(filterType)} ${filtersSelectors.filter_option(filterName)}`)
})

Then('Click on Update button for the {string} filter list', async function (filterType) {
    await this.page.click(`${filtersSelectors.filter_type(filterType)} ${filtersSelectors.update_button}`)
})

Then('The grid displays all meetings associated with the {string} {string}', async function (filterName, filterType) {
    const headerText = await this.page?.locator(filtersSelectors.column_header).last().textContent()
    expect(headerText?.toLowerCase()).toBe(filterType)

    const cells = await this.page.locator(`${filtersSelectors.table_rows} td:last-child`).allTextContents()
